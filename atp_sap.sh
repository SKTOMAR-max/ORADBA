...
curl -s https://raw.githubusercontent.com/SKTOMAR-max/ORADBA/refs/heads/main/DB_Patching_AutoScript/config.txt >  $PATCHTMP/config.txt

source_file="$PATCHTMP/config.txt"
...
echo "                  *             ---------   ----------     --------------  -------------------------------*"
echo "                  *             V 1.0       20/04/2025     SANDEEP TOMAR   Initial Version                *"
echo "                  *             V 1.1       30/04/2025     VINAY KUMAR     Updated Version                *"
echo "                  *                                                                                       *"
echo "                  *****************************************************************************************"
echo
} | tee -a $LOG_FILE
...
## Encrypted password
# encrypted_password="U2FsdGVkX1/5PYtgopuQZF9ijAeAgfF65291TmixyZI="
#
## Decryption key (store in ENV or ask user)
# decryption_key="encryptionkey"
#
## Decrypt the password
# decrypted_password=$(echo "$encrypted_password" | openssl enc -aes-256-cbc -a -d -salt -pass pass:$decryption_key)
#
## Prompt user
# read -sp "Enter password: " input_password
# echo
#
# if [[ "$input_password" == "$decrypted_password" ]]; then
#    echo "Access granted."
# else
#    echo "Access denied."
#    exit 1
# fi
#
# echo "Access granted. Running Patching script..."
...
display_menu() {
        echo -e "${CYAN}---------------------------------------------${RESET}"
		echo "Please Select Home for Patching: "
        echo "A. DATABASE HOME  "
        echo "B. OBSERVER "
        echo "C. EM_AGENT JDK "
        echo "D. Exit "
		echo -e "${CYAN}---------------------------------------------${RESET}"
...
while true; do
    ...
    # Read user input
    read -p "Enter your choice (A - D): " USER_CHOICE1 
    ...
    case $USER_CHOICE1 in
        A|a)
            echo
                        ...
                        echo -e "${CYAN} YOU CHOSE TO PATCH DATABASE HOME ${RESET}" | tee -a $LOG_FILE
                        echo -e "${CYAN}----------------------------------${RESET}" | tee -a $LOG_FILE
            echo
...
echo
##Unzip DB Patch S/W
#RU_ZIP="SAP19P_2411-70004508.ZIP"
...
while true; do
    ...
    # Read user input
    read -p "Enter your choice (1-11): " USER_CHOICE2 
...
curl -s $CURL_DB_PRECHECKS > $PATCHTMP/DB_Prechecks.sql
sqlplus -S / as sysdba @$PATCHTMP/DB_Prechecks.sql >> $LOG_FILE 2>&1
...
$ORACLE_HOME/OPatch/opatch lsinv|grep -i applied > $LOGS/POST_PATCH_INFO.txt
echo
...
curl -s $CURL_DB_POSTCHECKS >  $PATCHTMP/DB_Postchecks.sql
echo "@$PATCHTMP/DB_Postchecks.sql" | sqlplus -S / as sysdba >> $LOG_FILE
...
curl -s $CURL_EMAIL >  $PATCHTMP/email_db_status.sql
echo "@$PATCHTMP/email_db_status.sql" | sqlplus -S / as sysdba >> $email_file
    ...
    # The file that contains name and email
curl -s $CURL_VALID_USERS >  $PATCHTMP/valid_users.txt
file_path="$PATCHTMP/valid_users.txt"

# Search for the name and extract the email
        recipient2=$(awk -v n="$name" '$1 == n {print $2}' "$file_path")
    echo
    export TMPDIR="/tmp"
    ...
    if [ -f "$email_file" ]; then
        ...
        { echo "$email_body"; echo ""; echo "CHG No: $CHG_NO"; echo "";  echo ""; cat "$email_file"; echo "";  echo ""; cat "$LOGS/POST_PATCH_INFO.txt";} | mail -s "$subject" "$recipient1"  "$recipient2"
    else
...
echo "Removed patch biniaries!!!" | tee -a $LOG_FILE
    ...
    echo ""
        ...
        B|b)	
            echo
                        echo -e "${CYAN}----------------------------------${RESET}" | tee -a $LOG_FILE
                        echo -e "${CYAN} YOU CHOSE TO PATCH OBSERVER HOME ${RESET}" | tee -a $LOG_FILE
                        echo -e "${CYAN}----------------------------------${RESET}" | tee -a $LOG_FILE
            echo
...
curl -s $CURL_PULL_SCRIPT >  $PATCHTMP/pull_script.sh
chmod +x $PATCHTMP/pull_script.sh
$PATCHTMP/pull_script.sh "$LOG_FILE" "$PATCH_DIR" "$LOGS" "$PATCHTMP" &  # Running the inner script in the background
...
# Set the environment variable ORACLE_HOME for the session
# Ask for user's name
echo -n -e "${CYAN}PLEASE ENTER HOME LOCATION FOR OBSERVER (You have 60 Sec): ${RESET}"
read -t 60 OBSERVER
export ORACLE_HOME="$OBSERVER"
export PATH="$ORACLE_HOME/bin:$PATH"
echo
echo  | tee -a $LOG_FILE
echo -e "${CYAN}-----------------------------${RESET}"  | tee -a $LOG_FILE
...
display_menu() {
        echo -e "${CYAN}---------------------------------------------${RESET}"
		echo "Please select an option to perform: "
        echo "20. Take Observer Home Binary Backup "
        echo "21. Run Patch Conflicts "
		echo "22. Remove Inactive Patches from Observer Home "
        echo "23. Start Patching on Observer      =>BLOCK FOR PATCHING"
		echo "24. Remove Inactive Patches from Observer Home. Job will run in background "
        echo "25. Binary Cleanup "
        echo "26. Exit "
		echo -e "${CYAN}---------------------------------------------${RESET}"
    echo
}
...
while true; do
    ...
    # Read user input
    read -p "Enter your choice (20-26): " USER_CHOICE 
...
echo
echo -e "${CYAN}-----------------------------${RESET}" | tee -a $LOG_FILE
echo -e "${CYAN} OBSERVER HOME BINARY BACKUP ${RESET}" | tee -a $LOG_FILE
echo -e "${CYAN}-----------------------------${RESET}" | tee -a $LOG_FILE
                        echo
...
echo
echo -e "${CYAN}--------------------------${RESET}" | tee -a $LOG_FILE
echo -e "${CYAN} CHECKING PATCH CONFLICTS ${RESET}" | tee -a $LOG_FILE
echo -e "${CYAN}--------------------------${RESET}" | tee -a $LOG_FILE
echo
...
echo
 ...
 if [ $? -ne 0 ]; then
     echo "Error: There is a conflict with the JDK patch. Aborting patching." | tee -a $LOG_FILE
     echo "FAILED: JDK Bundle patch conflict..." | tee -a $LOG_FILE
     exit 1
 fi

 # Success message
 echo "PASSED: JDK Bundle patch conflict..." | tee -a $LOG_FILE
 echo
 for i in {1..5}; do
     echo -n "."
     sleep 1
...
echo
        ...
        echo
        echo -e "${CYAN}-------------------------------------------------------------------${RESET}" | tee -a $LOG_FILE
        echo -e "${CYAN} YOU CHOSE TO START REMOVING INACTIVE PATCHES FROM THE OBSERVER HOME ${RESET}" | tee -a $LOG_FILE
        echo -e "${CYAN}-------------------------------------------------------------------${RESET}" | tee -a $LOG_FILE
        echo
						# Step 1[A]: Listing Inactive Patches using OPatch
             echo "listing inactive patches..." | tee -a $LOG_FILE
 ...
 echo ""
        ...
        23)
            echo
                        echo -e "${CYAN}------------------------------------------${RESET}" | tee -a $LOG_FILE
                        echo -e "${CYAN} YOU CHOSE TO START THE OBSERVER PATCHING ${RESET}" | tee -a $LOG_FILE
                        echo -e "${CYAN}------------------------------------------${RESET}" | tee -a $LOG_FILE
                        echo
#Patching Steps
echo "Proceeding with Patching...." | tee -a $LOG_FILE
...
# Search for the name and extract the email
    ...
    if [ -f "$email_file" ]; then
        # Read the file and send its content as the email body using 'mail'
        { echo "$email_body"; echo ""; echo "CHG No: $CHG_NO"; echo "";  echo ""; echo "OBSERVER patching competed!!!"; echo "";  echo "";cat "$LOGS/POST_PATCH_INFO.txt";} | mail -s "$subject" "$recipient1"  "$recipient2"
    else
        echo "File not found: $email_file" | tee -a $LOG_FILE
    fi
...
echo "Removed patch biniaries!!!" | tee -a $LOG_FILE
        ...
        26)
            ...
            ;;
        *)
            echo "Invalid choice. Please select a number between 20 and 26."
            ;;
    esac
...
# Set the environment variable ORACLE_HOME for the session
# Ask for user's name
echo -n -e "${CYAN}PLEASE ENTER HOME LOCATION FOR EM_AGENT_HOME (You have 60 Sec): ${RESET}"
read -t 60 EM_AGENT_HOME
export ORACLE_HOME="$EM_AGENT_HOME"
export PATH="$ORACLE_HOME/bin:$PATH"
echo
echo  | tee -a $LOG_FILE
echo -e "${CYAN}-----------------------------${RESET}"  | tee -a $LOG_FILE
...
display_menu() {
        echo -e "${CYAN}---------------------------------------------${RESET}"
		echo "Please select an option to perform: "
        echo "30. Create Blackout & Bring down EM Agent "
        echo "31. Apply JDK on Agent & Start Agent "
		echo "32. Stop Blackout "
		echo "33. Remove Inactive Patches from Observer Home. Job will run in background "
        echo "34. Binary Cleanup "
        echo "35. Exit "
		echo -e "${CYAN}---------------------------------------------${RESET}"
    echo
}
...
while true; do
    ...
    # Read user input
    read -p "Enter your choice (30-35): " USER_CHOICE 
...
31)
 echo
                        echo -e "${CYAN}---------------------------------------------${RESET}" | tee -a $LOG_FILE
                        echo -e "${CYAN} YOU CHOSE TO START JDK PATCHING ON EM AGENT ${RESET}" | tee -a $LOG_FILE
                        echo -e "${CYAN}---------------------------------------------${RESET}" | tee -a $LOG_FILE
                        echo
						echo -e "${CYAN}Pulling Agent JDK patches...${RESET}" | tee -a $LOG_FILE
# Ask the user if they want to proceed with downloading the patch
read -p "Do you want to proceed with downloading the patch? (yes/no): " response
...
if [[ "$response" == "yes" ]]; then
    ...
    echo
curl -s $CURL_PULL_SCRIPT >  $PATCHTMP/pull_EM_JDK.sh
chmod +x $PATCHTMP/pull_EM_JDK.sh
$PATCHTMP/pull_EM_JDK.sh "$LOG_FILE" "$PATCH_DIR" "$LOGS" "$PATCHTMP" &  # Running the inner script in the background
...
# Unzip the patch software
                                (
                                        yes y | unzip -q "$PATCH_DIR/$EM_JDK_ZIP" >> "$PATCH_DIR/EM_JDK.txt" 2>&1 &
                                ...
                                echo "EM_JDK Unzip completed. Check the log at $LOG_FILE"
                                echo
                                # Exit the script

                                # Check if unzip was successful
                                if [ $? -eq 0 ]; then
                                    echo "EM_JDK software unzipped successfully." | tee -a $LOG_FILE
                                else
                                    echo "Error unzipping EM_JDK software." | tee -a $LOG_FILE
                                    exit 1  # Exit if unzip fails
                                fi
...
echo "-----------------------------------------------------" | tee -a $LOG_FILE
echo
$ORACLE_HOME/AgentPatcher/agentpatcher lspatches > $PATCH_DIR/lspatches.bck
$ORACLE_HOME/oracle_common/jdk/bin/java -version | tee -a $LOG_FILE
cp $EM_JDK_PATCH $ORACLE_HOME/oracle_common/
cd $ORACLE_HOME/oracle_common/
mv jdk $PATCH_DIR/JDK_OLD_$DATE
tar -zxvf $EM_JDK_PATCH
mv $EM_JDK jdk
$ORACLE_HOME/oracle_common/jdk/bin/java -version > $LOGS/AGENT_JDK_INFO.txt
sleep 5
echo
echo -n -e "${CYAN}Are we applied correct patch?  If YES the good to proceed, If NO the exit from the script and reapply correct patches. (yes/no): ${RESET}"
read answer
...
;;
		...
		echo
            echo -e "${CYAN}-------------------------------------------------${RESET}" | tee -a $LOG_FILE
            echo -e "${CYAN} YOU CHOSE TO START AGENT & STOP BLACKOUT OPTION ${RESET}" | tee -a $LOG_FILE
            echo -e "${CYAN}-------------------------------------------------${RESET}" | tee -a $LOG_FILE
            echo
                        echo "Removing Blackout from EM Agent....." | tee -a $LOG_FILE
                        echo
...
rm -rf $LOGS/*
rm -rf $PATCHTMP/*
rm -rf $PATCH_DIR/p* $PATCH_DIR/$EM_JDK_PATCH_ID $PATCH_DIR/JDK_OLD*
echo
echo "Removed patch biniaries!!!" | tee -a $LOG_FILE
...
# Search for the name and extract the email
    ...
    if [ -f "$email_file" ]; then
        # Read the file and send its content as the email body using 'mail'
        { echo "$email_body"; echo ""; echo "CHG No: $CHG_NO"; echo "";  echo ""; echo "EM Agent patching competed!!!"; echo "";  echo "";cat "$LOGS/AGENT_JDK_INFO.txt";} | mail -s "$subject" "$recipient1"  "$recipient2"
    else
        echo "File not found: $email_file" | tee -a $LOG_FILE
    fi
...
D|d)
            ...
            ;;
        *)
            echo "Invalid choice. Please select a letter between A and C."
            ;;
    esac
